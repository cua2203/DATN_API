...
export class VariantController {
  ...
  async getAllPaging(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const data = await this.service.getAllPaging(
        JSON.stringify(search_criteria),
      );
      if (data && data.length > 0) {
        res.json(data);
      } else {
        res.json({ message: ' Danh sách trống', data:[] });
      }
    } catch (error: any) {
      next(error);
    }
  ...
  async getAllVariant(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  async getAllVariantWithoutPrice(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
